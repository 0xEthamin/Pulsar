...
//! boards speak, the reader the processing board polls the control link
//! through, the gate a control message passes before it reaches audio, the
...
//! widening of each sample into its slot. It holds the controls of that board
//! too: the coarse step of the encoder, the fine value the phone owns, and the
//! schedule that puts both on the control link. A plan carries the field
//! encodings, the bounds and the read-back comparison, so the firmware that
//! writes the registers is the register block and nothing else, and every rule
//! is tested on a host. Nothing here touches a peripheral.
