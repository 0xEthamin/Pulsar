...
//! processing board leaves for a debugger, the two hardware plans that board
//! runs on, the audio clock and the output transport, and the gate that raises
//! the converter mute line once every stage feeding the converters has
//! reported. A plan carries the field encodings, the bounds and the read-back
